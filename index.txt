...
const fs = require('fs');
const spawn = require("child_process").spawn; let extractedText21212;
(async () => {
    ...
    await page.evaluate(() => {
        document.querySelector('#structured_composer_async_container > div:nth-child(4) > a > span').click();
    });
    await page.waitForTimeout(5000);
    ...
    for (var i = 0; i < elHandleArray.length; i++) {
        ...
        await npage.goto(href); await npage.waitForTimeout(10000);
        ...
        await npage.waitForTimeout(10000);
        const extractedText2121 = await npage.$eval('*', (el) => el.innerText);
        console.log(extractedText2121); extractedText21212 = extractedText2121 + extractedText2 + extractedText1;
    ...
    const w = ["National Geography", "Discovery Channel","Siddhant Majumder", "Nat Geo Wild", "Love", "desmos", "Swagatam Banerjee"];
    let wild = new Map();
    ...
    let x = extractedText21212.split('\n');
